...
const dispatch = useDispatch();
  ...
  const searchMovieName=async(gptMovieArrayText)=>{
   ...
   const results = await Promise.all (uniqueMovies.map(async(movie)=>{
  ...
  return (
    <div className='pt-64 md:pt-36 m-8 '>
      ...
      >
        <input
          type='text'
          placeholder={languageConstant[langSel]?.Placeholder || 'Enter your movie preferences'}
          className='mr-4 p-4 rounded-lg w-full flex'
          ref={inputRef}
        />

        <button
          className='bg-red-700 text-white px-8 py-4 rounded-lg m-auto w-1/2 mt-4'
        >
